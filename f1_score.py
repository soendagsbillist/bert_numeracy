...
val_generator = DataLoader(validation_data, batch_size=8, collate_fn=lambda x: x,
                           num_workers=2)
precisions = []
recalls = []

for index, data in enumerate(tqdm(val_generator)):
    for q in data:
        ...
        if 'span' or 'date' in list(list(zip(*answer['types']))[0]):
            ...
            if num_same or len(normalized_prediction) == 0:
              precision = 0
              recall = 0
            else:
